...
import { toggleFavorites } from '../../app/favoriteSlice'
import { CatSingle } from '../../utils/types'

const Favorites= ({cat}: CatSingle) => {

    const dispatch = useAppDispatch()
    const favorites = useAppSelector(state => state.favorite)
    const isExist = favorites.some(r => r.id ==cat.id)

  return <div className='cat-favorites'>
  <svg xmlns="http://www.w3.org/2000/svg"  width="48" height="48" viewBox="0 0 48 48" fill="#F24E1E">
  ...
  <svg xmlns="http://www.w3.org/2000/svg" 
      onClick={() => {dispatch(toggleFavorites(cat))}} 
      width="48" height="48" viewBox="0 0 48 48" fill={isExist  ? '#F24E1E' : 'transparent'}>
